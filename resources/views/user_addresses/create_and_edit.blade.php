@extends('layouts.app')
@section('title', ($address->id ? '修改': '新增') .'新增收货地址')
                        ...
                        @if($address->id)
                            <form class="form-horizontal" role="form" action="{{ route('user_addresses.update', ['user_address' => $address->id]) }}" method="post">
                                ...
                                @else
                                    ...
                                    {{ csrf_field() }}
                        <!-- 注意这里多了 @change -->
                            <select-district :init-value="{{ json_encode([old('province', $address->province), old('city', $address->city),
                             old('district', $address->district)]) }}" @change="onDistrictChanged" inline-template>
